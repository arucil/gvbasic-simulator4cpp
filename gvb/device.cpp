...
void Device::loadFile(const char *fn, uint8_t *p, size_t n) {
   FILE *fp = fopen(fn, "rb");
   if (!fp)
      return;
   fread(p, n, 1, fp);
...
uint8_t Device::getKey() {
   ...
   do {
      {
         shared_lock<shared_timed_mutex> lock(m_mutMem);
   ...
   m_gui->m_displayCursor = false;
   {
      unique_lock<shared_timed_mutex> lock(m_mutMem);
      *m_memKey &= 127;
   }
...
void Device::poke(uint16_t addr, uint8_t value) {
   ...
   if (addr >= m_memGraph - m_mem && addr <= m_memGraph - m_mem + 1600) {
      addr -= m_memGraph - m_mem;
      int x = addr % 20;
      int y = addr / 20;
      m_gui->update(x << 3, y, x + 1 << 3, y);
   } else if (addr == m_memKey - m_mem && value == 155) {
      throw Quit();
   }
}
